...
const greenJobInZipCode = (data: Job[], zipCode: string): boolean => data.some((item) => item.address.includes(zipCode));

const listIndustries = (data: Job[]): string[] => [
  ...new Set(data.map((item) => item.industry)),
];

const jobsWithKeyword = (data: Job[], keyword: string): string[] => data
...
const maxIndustryJobs = (data: Job[]): IndustryCount | null => {
  ...
  return industries.reduce((max, current) => (current.jobs > max.jobs ? current : max));
};

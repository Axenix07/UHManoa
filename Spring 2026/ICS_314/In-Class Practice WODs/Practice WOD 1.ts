...
function windchill(temp: number, wind: number): string {
  ...
  if (windchill <= -48) {
    warning = 'Extreme danger of frostbite possible on exposed skin within 5 minutes.';
  } else if (windchill <= -32) {
    warning = 'Very high danger of frostbite possible on exposed skin within 10 minutes';
  } else if (windchill < -17) {
    warning = 'High danger of frostbite possible on exposed skin within 30 minutes';
  } else if (windchill <= 25) {
    warning = 'Moderate danger of frostbite possible on exposed skin over 2 hours.';
  } else {
    warning = 'None';
  }
